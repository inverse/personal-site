...
Out of the box the nvidia GPU drivers are not installed. At the time of writing the current stable drivers for the GeForce GTX 960M is the `nvidia-367` series. First you must get the nvidia ppa.
...
You will have to disable secure boot from within the BIOS to enable the installation of 3rd party drivers. Or by following the on screen instructions to do so.

Along with installing the nvidia drivers I recommend installing `nvidia-prime` too as it'll enable you to switch between the Intel and Nvidia GPU easily from within unity.
